...
def make_dataset(mode, root):
    if mode == "train":
        img_path = os.path.join(root, "trainval")
        mask_path = os.path.join(root, "processed_trainvalannot")
...
class TN3K_al(data.Dataset):
    def __init__(self, quality, mode, data_path='', joint_transform=None,
        ...
        splits = np.load('data/tn3k_al_splits.npy', allow_pickle=True).item() #waiting
        # print(splits)
        ...
        #     self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['d_v'])]
            ...
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits_sum)]
            print(len(splits['p_t']),len(splits['t_q']))

...
class Camvid_al(data.Dataset):
    def __init__(self, quality, mode, data_path='', joint_transform=None,
                 ...
                 region_size=(80, 90), pretrain = False,
        ...
        splits = np.load('data/camvid_al_splits_pretrain.npy', allow_pickle=True).item()
        ...
        if split == 'train':
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['d_t'])] #100
        elif split == 'test':
            ...
            # print(1111111)
            splits_sum = splits['d_t']+splits['p_t']
    ...
    def __getitem__(self, index):
        ...
        if self.joint_transform is not None:
            if not (self.candidates or self.pretrain or self.split == 'unlab'):

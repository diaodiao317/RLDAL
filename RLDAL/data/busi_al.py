...
class BUSI_al(data.Dataset):
    def __init__(self, quality, mode, data_path='', joint_transform=None,
                 ...
                 region_size=(64, 64), pretrain = False,
                 num_each_iter=1, only_last_labeled=True, split='train',re_all_length = False):
        ...
        splits = np.load('data/busi_al_splits.npy', allow_pickle=True).item() #waiting
        self.state_subset = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['d_s'])] #10
        self.state_subset_regions = {}
        for i in range(len(splits['d_s'])):
            x_r1 = np.arange(0, 512 - region_size[0] + 1, region_size[0])
            y_r1 = np.arange(0, 512 - region_size[1] + 1, region_size[1])
            self.state_subset_regions.update({i: np.array(np.meshgrid(x_r1, y_r1)).T.reshape(-1, 2)})
    
        if split == 'select_unlab_region':
            splits_sum = splits['d_t'] + splits['d_v'] 
            self.imgs = self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits_sum)] #挑选集
        elif split == 'test':
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['d_v'])]
        elif split == 'pretrain':
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['p_t'])]
        elif split == 'train':
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits['t_q'])]
        elif split == 'unlab':
            # print(1111111)
            splits_sum = splits['d_t']+splits['t_q']
            # print(len(splits_sum))
            self.imgs = self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits_sum)]
        elif split == 'unlab_final':
            splits_sum = splits['d_t'] + splits['d_v'] 
            self.imgs = self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits_sum)]
        elif split == 'full_sup_final':
            splits_sum = splits['d_v'] + splits['p_t'] + splits['t_q']
            self.imgs = [img for i, img in enumerate(self.imgs) if (img[-1] in splits_sum)]
            print(len(splits['d_v']),len(splits['p_t']),len(splits['t_q']))
        ...
        splitters_x = np.arange(0, 512 - region_size[0] + 1, region_size[0])
        splitters_y = np.arange(0, 512 - region_size[1] + 1, region_size[1])
        ...
        self.num_unlabeled_regions_total = (512 * 512) // (
    ...
    def __getitem__(self, index):
        ...
        if self.joint_transform is not None:
            if not (self.candidates or self.pretrain or (self.split == 'unlab' or self.split == 'unlab_final')):
        ...
        if self.target_transform is not None:
            mask = self.target_transform(mask)
        if self.split == ('unlab' or 'unlab_final'):

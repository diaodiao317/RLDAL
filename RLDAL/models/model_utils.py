...
def compute_state(args, net, region_candidates, candidate_set, train_set, num_groups=5, reg_sz=(128, 128)):
    ...
    for candidates in region_candidates:
        ...
        # Adding action representation
            ...
            # ent_arr.append(torch.mean(ent_region))
    ...
    if args.al_algorithm == 'ralis':
        ...
        return all_state, region_candidates,0
    ...
    if args.al_algorithm == 'ralis':
        ...
        if sample > eps_threshold or test:
            ...
            with torch.no_grad():
                ...
                # print('111111',q_val_.shape)
                index = 1
                action = q_val_.max(index)[1].view(-1)
                # print(action)
                # print(state.shape)
                # print(action.shape)
                # print(q_val_.max(index)[0].view(-1).cpu())
                # print(state[torch.arange(state.size(0)), action].shape)
                # print(state[torch.arange(state.size(0)), action][:,-1])
                # if test == True and q_val_.max(index)[0].view(-1).cpu().min() < 0:
                #     reset_net = True
                # print(q_val_.shape , state_ent.shape)
                state_ent = state_ent.to(device).float().squeeze()
                # new_sel = q_val_ 
                # index = q_val_ < 0
                # index2 = q_val_ >0
                # print(index.shape,q_val_.shape,state_ent.shape)
                # new_sel1 =  q_val_ * (state_ent* -1) *index
                # new_sel2 =  q_val_ * (state_ent) *index2
                state_ent1 = state_ent
                ...
                for i in range(state_ent.size(0)):
                    ...
                    topk_vals, topk_idx = torch.topk(ent_row[pos_idx], k=keep_k, largest=True, sorted=False)
                    keep_indices = pos_idx[topk_idx]
                    keep_mask[i, keep_indices] = True
                state_ent = state_ent * keep_mask.float()
                
                
                # q_val_avg = q_val_.mean(dim=1, keepdim=True)
                # q_flag = q_val_ > q_val_avg
                # q_val_ = q_val_avg * q_flag
...
# 逐行归一化 (safe)
                ...
                # Debug: print per-candidate Q (normalized), entropy, and mixed score
                # try:
                #     q_cpu = normalized_q_val_.detach().cpu().tolist()
                #     ent_cpu = state_ent.detach().cpu().tolist()
                #     mix_cpu = new_sel.detach().cpu().tolist()
                #     for i in range(len(q_cpu)):
                #         print(f"group {i}: q_norm={q_cpu[i]} ent={ent_cpu[i]} q*ent={mix_cpu[i]}")
                # except Exception as e:
                #     print(f"debug print failed: {e}")
                # new_sel = normalized_q_val_ * state_ent1
                # print(new_sel)
                ...
                for i in range(q_val_.size(0)):
                    ...
                    if action[i] >= 0:
                        ...
                        if ent_max > 0 and sel_ent < 0.5 * ent_max:
                            reset_net = True
                            break
                # print(new_sel.max(1)[0].view(-1).cpu())
                print('this is select by dqn')
                # print(state[:, :, -1].shape)
                # max_ent2 = torch.max(state_ent[:, :, -1], dim=1)
                # action2 = max_ent2[1]  # Index where the maximum entropy is.
                # ent2 = max_ent2[0]
                # print(action2,ent2)
                # print(q_val_[torch.arange(q_val_.size(0)), action2])
                # del (q_val_)
                del (state)
            action = action.cpu()
        else:
...
def compute_entropy_seg(args, im_t, net):
    ...
    with torch.no_grad():
        out, _ = net(im_t)
        out_soft_log = F.log_softmax(out, dim=1)
        out_soft = torch.exp(out_soft_log)
        ent = - torch.sum(out_soft * out_soft_log, dim=1).detach()
        del (out)
        del (out_soft_log)
        del (out_soft)
        del (im_t)

...
def get_arguments():
    ...
    parser.add_argument("--exp-name", type=str, default='LA_supervised',
    ...
    parser.add_argument("--gamma-scheduler-dqn", type=float, default=0.99,
                        help="Coefficient to decrease learning rate with "
                             "exponential scheduler."
                             " lr = old_lr * (gamma**epochs)")
    ...
    parser.add_argument("--input-size", nargs='+', type=int,
                        default=(256, 256), help="Crop size,"
    ...
    parser.add_argument("--region-size", nargs='+', type=int, default=(64, 64))

    parser.add_argument("--lr-dqn", type=float, default=0.0001)

    parser.add_argument("--labeled-fraction", type=float, default=0.05,
                        help="Fraction of labeled samples to keep when running semi-supervised training (0-1].")
    parser.add_argument("--consistency-weight", type=float, default=1.0,
                        help="Weight applied to the unlabeled consistency loss term during EMA training.")
    parser.add_argument("--confidence-threshold", type=float, default=0.6,
                        help="Confidence threshold for accepting teacher pseudo-labels in unlabeled batches.")
    ...
    parser.add_argument("--dqn-gamma", type=float, default=0.9)
    parser.add_argument("--dqn-epochs", type=int, default=1)
    parser.add_argument("--dqn-action-select", type=str, default='softmax', choices=['epsilon', 'softmax'],
    ...
    parser.add_argument("--final-sup-only", action='store_true',
                        help="After AL budget is reached, skip EMA/semi-supervised finetune and run supervised-only finetune on labeled regions")
...
def save_arguments(args):
    ...
    if args.train or args.al_algorithm == 'ralis':
        target_root = getattr(args, 'run_workspace', None)
        if target_root is not None:
            path = os.path.join(target_root, 'args.json')
        else:
